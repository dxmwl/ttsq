...
import com.shengqiangou.mobile.manager.ActivityManager
import com.shengqiangou.mobile.http.exception.ResultException
import com.shengqiangou.mobile.http.exception.TokenException
import com.shengqiangou.mobile.ui.activity.LoginActivity
...
class RequestHandler constructor(private val application: Application) : IRequestHandler {
    ...
    override fun requestSucceed(httpRequest: HttpRequest<*>, response: Response, type: Type): Any {
        ...
        if (result is HttpData<*>) {
            ...
            if (model.isTokenFailure()) {
                // 代表登录失效，需要重新登录
                throw com.shengqiangou.mobile.http.exception.TokenException(application.getString(R.string.http_token_error))
            }
            throw com.shengqiangou.mobile.http.exception.ResultException(model.getMessage(), model)
        }
        return result
    }
    ...
    override fun requestFail(httpRequest: HttpRequest<*>, e: Exception): Exception {
        ...
        if (e is HttpException) {
            if (e is com.shengqiangou.mobile.http.exception.TokenException) {
                // 登录信息失效，跳转到登录页
    ...
    override fun readCache(httpRequest: HttpRequest<*>, type: Type, cacheTime: Long): Any? {
        val cacheKey: String? = com.shengqiangou.mobile.http.model.HttpCacheManager.generateCacheKey(httpRequest)
        val cacheValue: String? = com.shengqiangou.mobile.http.model.HttpCacheManager.getMmkv().getString(cacheKey, null)
    ...
    override fun writeCache(httpRequest: HttpRequest<*>, response: Response, result: Any): Boolean {
        ...
        EasyLog.printLog(httpRequest,"---------- cacheValue ----------")
        EasyLog.printJson(httpRequest,cacheValue)
        return com.shengqiangou.mobile.http.model.HttpCacheManager.getMmkv().putString(cacheKey, cacheValue).commit()
    }
}