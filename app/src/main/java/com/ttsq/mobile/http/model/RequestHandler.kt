...
import com.ttsq.mobile.http.exception.ResultException
import com.ttsq.mobile.http.exception.TokenException
...
class RequestHandler constructor(private val application: Application) : IRequestHandler {
    ...
    override fun readCache(httpRequest: HttpRequest<*>, type: Type, cacheTime: Long): Any? {
        val cacheKey: String? = com.ttsq.mobile.http.model.HttpCacheManager.generateCacheKey(httpRequest)
        val cacheValue: String? = com.ttsq.mobile.http.model.HttpCacheManager.getMmkv().getString(cacheKey, null)
    ...
    override fun writeCache(httpRequest: HttpRequest<*>, response: Response, result: Any): Boolean {
        ...
        EasyLog.printLog(httpRequest,"---------- cacheValue ----------")
        EasyLog.printJson(httpRequest,cacheValue)
        return com.ttsq.mobile.http.model.HttpCacheManager.getMmkv().putString(cacheKey, cacheValue).commit()
    }
}